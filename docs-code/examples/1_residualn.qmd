...
- _Global signal regression_: Regression out of the mean time series signal from the time series data (common in imaging neuroscience).
...
fig_imshow(W[np.ix_(ordw, ordw)], "Original structural network", "inferno").show()
fig_imshow(C[np.ix_(ordc, ordc)], "Original correlation network", "viridis").show()
...
We now visualize variants of the residual structual and correlation networks.

```{python}

fig_imshow(Wd[np.ix_(ordw, ordw)], "Structural network after degree correction", "inferno").show()
fig_imshow(Wr[np.ix_(ordw, ordw)], "Structural network first-component removal", "inferno").show()
fig_imshow(Cd[np.ix_(ordc, ordc)], "Correlation network after degree correction", "viridis").show()
fig_imshow(Cg[np.ix_(ordc, ordc)], "Correlation network after global signal regression", "viridis").show()
...
fig_scatter(Wr[not_eye], Wd[not_eye], 
            ...
            f"Structural networks (r = {np.corrcoef(Wr[not_eye], Wd[not_eye])[0, 1]:.3f})").show()

fig_scatter(Cd[not_eye], Cg[not_eye], 
            "Network weights after degree correction",
            "Network weights after global-signal regression",
            f"Correlation networks (r = {np.corrcoef(Cd[not_eye], Cg[not_eye])[0, 1]:.3f})").show()
