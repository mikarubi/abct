...
def step2_test(X, W, n, k, Args):
    ...
    if k > n:
        raise ValueError(
            "Number of modules must not exceed number of nodes or data points."
        )
    if Args.numbatches > n:
        raise ValueError(
            "Number of batches must not exceed number of nodes or data points."
        )
    ...
    if Args.method == "loyvain":
        ...
        if not (
            W.shape[0] == W.shape[1]
            and np.allclose(W, W.T, atol=np.finfo(np.float32).eps)
        ):
            raise ValueError(
        ...
        if Args.start == "custom":
            if not (
                len(Args.M0) == n and np.array_equal(np.unique(Args.M0), np.arange(1, k + 1))
            ):
                raise ValueError(
                    f"Initial module assignment must have length {n} and contain integers 1 to {k}."
                )
