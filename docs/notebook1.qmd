...
WD["Co-neighbor"] = abct.kneighbor(W, "common", kappa=0.1).todense()
...
CD["Co-neighbor"] = abct.kneighbor(C, "common", kappa=0.1).todense()
...
for i, name in enumerate(WD):
    fig = px.imshow(WD[name][np.ix_(ordw, ordw)])
...
for i, name in enumerate(CD):
    fig = px.imshow(CD[name][np.ix_(ordc, ordc)])
